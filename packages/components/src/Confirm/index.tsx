import { CheckCircle, Close, CloseCircle, QuestionCircle, WarningCircle } from '@kun-ui/icons';
...
const Confirm = React.forwardRef<any, Props>(
  (
    ...
    return (
      <Modal {...props} ref={ref}>
        <Zoom in={open}>
          <DialogWrap theme={theme} className={clsx('KunConfirm-root', className)} style={style}>
            <Close className="KunConfirm-close" onClick={onClose} />
            <DialogTitleWrap
              hasDesc={Boolean(desc)}
              theme={theme}
              type={type}
              className="KunConfirm-title-wrap"
            >
              <Icon className="KunConfirm-icon" theme={theme} />
              <DialogTitle theme={theme} className="KunConfirm-title">
                {title}
              </DialogTitle>
            </DialogTitleWrap>
            <DialogDesc theme={theme} className="KunConfirm-desc">
              {desc}
            </DialogDesc>
            <DialogFooter>
              {type === 'question' || type === 'warning' ? (
                <>
                  ...
                  <Button
                    className={clsx({ 'KunConfirm-cancel-btn': !equal })}
                    variant={equal ? 'outlined' : 'contained'}
