...
const Wrap = styled.div<PropsWithoutSize>`
  ...
  &:focus-within:not(.Kun-disabled) {
    &:after {
      transform: scaleX(1);
    }
    .KunTextField-input-placeholder {
...
const TextField: React.FC<Props> = (
  ...
  return (
    <Wrap
      {...props}
      className={clsx(props.className, { 'Kun-disabled': disabled })}
      size={size}
      sizeOpt={sizeOpt}
      disabled={disabled}
    ...
    >
      {prefix}
      {!nativePlaceholder && <span className="KunTextField-input-placeholder">{placeholder}</span>}
      <InputBase
        {...inputProps}
        className={clsx('KunTextField-input', inputProps.className)}
...
export default withKunTheme(TextField, 'KunTextField-root');
